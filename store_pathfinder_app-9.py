...
# 5 aisles, 24 products (A3 closed), 12 items each, banana-leaf blocked lanes
# ============================================================================
...
class StoreNavigationProblem(Problem):
    ...
    def actions(self, state):
        return [n for n in self.neighbors.get(state, []) if n not in self.blocked and n not in PERMANENTLY_BLOCKED]
...
# ============================================================================
# PERMANENTLY BLOCKED AISLE
# ============================================================================
PERMANENTLY_BLOCKED = {'A3'}  # Aisle 3 is closed -- no entry allowed
...
PRODUCT_MAP = {
    ...
    # A3 (Meat) is permanently closed -- no products available
...
def render_svg(a_pos, b_pos, a_collected, b_collected, a_list, b_list):
    ...
    for nd, (x,y) in pos.items():
        ...
        # Permanently closed aisle overlay
        if nd in PERMANENTLY_BLOCKED:
            s.append(f'<circle cx="{x}" cy="{y}" r="{r+12}" fill="#3d0000" opacity="0.7"/>')
            s.append(f'<text x="{x}" y="{y+4}" text-anchor="middle" font-size="20">🚫</text>')
            s.append(f'<text x="{x}" y="{y-r-8}" text-anchor="middle" fill="#ff4444" '
                     f'font-size="9" font-weight="bold" font-family="sans-serif">CLOSED</text>')
    ...
    s.append(f'<text x="15" y="{ly}" fill="#7d8590" font-size="10" font-family="sans-serif">'
             ...
             f'🚫 CLOSED = permanently blocked aisle  |  '
...
def init_game(entrance, algo_b, player_a_mode='manual'):
    ...
    st.session_state.gs = {
        ...
        'a_list': sorted(prods[:12]), 'b_list': sorted(prods[12:24]),
        'a_got': [], 'b_got': [],
...
def process_move(gs, choice):
    ...
    gs['turn'] += 1
    # Check win
    if len(gs['a_got'])>=12 or len(gs['b_got'])>=12:
        gs['game_over'] = True
        a_d, b_d = len(gs['a_got'])>=12, len(gs['b_got'])>=12
        if a_d and b_d:
...
if 'gs' not in st.session_state or not st.session_state.gs.get('active'):
    ...
    st.markdown(render_svg('ENTRANCE_TOP','ENTRANCE_BOT',[],[],
                           list(PRODUCT_MAP.keys())[:12],list(PRODUCT_MAP.keys())[12:]),
                unsafe_allow_html=True)
    ...
    with r1:
        ...
        4. Each player gets **12 random items** -- first to collect all wins!
        """)
    with r2:
        st.markdown("### 🗺️ Aisle Directory")
        for a in AISLE_NODES:
            if a in PERMANENTLY_BLOCKED:
                st.markdown(f"**🚫 CLOSED** (`{a}`): *Aisle 3 is permanently closed -- no entry or products*")
            else:
...
with s1:
    st.metric("🟡 Your Items", f"{len(gs['a_got'])}/12")
with s2:
    st.metric("🟡 Your Moves", gs['a_moves'], delta=f"{gs['a_waits']} waits")
with s3:
    st.metric(f"🟣 AI Items ({gs['algo']})", f"{len(gs['b_got'])}/12")
with s4:
...
if gs['game_over']:
    if gs['winner'] == 'You':
        st.balloons()
        st.success(f"## 🎉 YOU WIN!  \nCollected 12 items in **{gs['a_moves']} moves** "
                   f"({gs['a_waits']} waits)")
    elif gs['winner'] == 'AI':
        st.error(f"## 🤖 AI Wins!  \nCollected 12 items in **{gs['b_moves']} moves** "
                 f"using {gs['algo']}")
    else:
        st.info("## 🤝 It's a TIE!")
    ...
    blocked_options = []
    for n in neighbors:
        if n in PERMANENTLY_BLOCKED:
            continue  # Aisle 3 permanently closed -- skip entirely
        if n == blocked_node:
